...
*/
		...
		FROM
			#Plans2Check AS p
		WHERE
			NonSargeableExpressionWithJoin = 1;

		SET @AdditionalInfo = 
							(
								SELECT
									N'Query Plans with Non-SARGable Expression with Join'	AS AdditionalInfo,
									@NumberOfPlans											AS NumberOfPlans
								WHERE
									@NumberOfPlans > 0
								FOR XML
									PATH (N'') ,
									ROOT (N'QueryPlanswithNonSargeableExpressionWithJoin')
							);
